...
export const useLikeThreadsMutation = ({ config }) => {
  const { post } = useRequest();
  const likeThreads = async ({ user_id, threads_id }) => {
    const STATION_HOSTNAME = window.location.hostname;
    config;
    try {
      const response = await post(
        `http://${STATION_HOSTNAME}:8000/likeThreads/like`,
        {
          json: {
            user_id,
            threads_id,
  ...
  return useMutateData(likeThreads, {
    ...config,
  });
};
